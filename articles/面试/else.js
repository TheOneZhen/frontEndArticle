  ...
  then(onFulfilled, onRejected) {
    ...
    let promise2 = new Promise((resolve, reject) => {
      const fulfilled = () => setTimeout(() => {
        try { resolvePromise(promise2, onFulfilled(this.value), resolve, reject) } catch (error) { reject(error) }
      })
      const rejected = () => setTimeout(() => {
        try { resolvePromise(promise2, onRejected(this.reason), resolve, reject) } catch (error) { reject(error) } 
      });
