响应式数据`data`改变会触发副作用函数`effect`，`effect`是更新视图或者更新其他响应式数据等操作的封装。在触发`effect`之前，需要进行`data`与`effect`关系建立，即`track`；关系建立后触发`effect`称为`trigger`。响应系统设计主要任务就是解决它们之间边界问题。
