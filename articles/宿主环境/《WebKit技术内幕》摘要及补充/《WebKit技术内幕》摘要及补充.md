...
<!-- - 浏览器：Chromium（开源）、Chrome、Safari

    Chromium是Google开源的浏览器，本文中很多地方都是围绕Chromium介绍的。

- 渲染引擎：WebKit、Blink

    WebKit是由Apple公司提出的开源项目，后续发展中，Google从WebKit项目独立出了Blink项目。一开始俩个项目差别不大（直接拷贝，删除了无关代码），之后各自发展。WebKit主要包含俩个模块：WebCore和JS引擎
