...
# YDKJSYY
1. 引擎、编译器和作用域
   1. 引擎负责编译和执行JS程序，编译器处理所有的解析和代码生成，作用域收集并维护一张所有被声明的标识符列表并执行隔离。
   2. 在处理程序`var a = 2;`时，首先编译器入场，去特定的作用域集合中查找`var a`，如果找到了就忽略声明并继续前进，否则让作用域声明一个新变量；然后编译器将`var a`变成引擎可执行的代码，引擎通过查找作用域集合判断是否有`a`这个元素，如果有则使，否则抛出`Reference Error`
2. ch1 line 142，这样是不是意味着函数声明和变量声明的函数变量提升具有相同优先级？
3. 
